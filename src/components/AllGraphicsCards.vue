<template>
  <q-page class="pageBackground">
    ...
    <header class="header-negative-info">
          ...
          >
            ...
            <div class="feature-box">
              ...
              <div class="feature-box__heading merriweather-regular-italic">
                <div class="feature-box__heading-span card__heading-span--2">
                  {{ item.productName }}
                </div>
              </div>
...
export default {
  setup() {
    ...
    const loadGraphics = async () => {
      try {
        state.graphics = await fetcher(`Category/1`);
        state.status = `Loaded ${state.graphics.length} Items for Laptops`;
      } catch (err) {
        console.log(err);
        state.status = `Error has Occured: ${err.message}`;
      }
    ...
    const addToCart = async (cardid) => {
      state.selectedCard = state.graphics.find((item) => item.id === cardid);
      ...
      if (state.cart.length > 0) {
        index = state.cart.findIndex(
          ...
          (item) => item.id === state.selectedCard.id
        );
      }
