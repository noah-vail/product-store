<template>
  <q-page class="pageBackground">
    ...
    <header class="header-secondary-accent">
    ...
    <q-card class="products">
      <q-card class="products__container">
        <div class="heading-tertiary" style="padding-left: 1.5rem">
          ALL LAPTOPS
        </div>
        <div class="row roww">
          <q-card-section
            class="col-1-of-3"
            v-for="item in state.laptops"
            :key="item"
          >
            <!--  -->
            <div class="feature-box">
              ...
              <div class="feature-box__heading merriweather-regular-italic">
                <div class="feature-box__heading-span card__heading-span--1">
                  {{ item.productName }}
                </div>
              </div>
...
export default {
  setup() {
    ...
    const loadLaptops = async () => {
      try {
        state.laptops = await fetcher(`Category/2`);
        state.status = `Loaded ${state.laptops.length} Items for Laptops`;
      } catch (err) {
        console.log(err);
        state.status = `Error has Occured: ${err.message}`;
      }
    ...
    const addToCart = async (productid) => {
      state.selectedProduct = state.laptops.find(
